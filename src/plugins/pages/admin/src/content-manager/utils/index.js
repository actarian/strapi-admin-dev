    ...
    if (!tmp) {
      Object.keys(components).map((name) => {
        if (components[name].isComponent) {
          Object.keys(components[name].attributes).map((field) => {
            if (components[name].attributes[field].component && components[name].attributes[field].component === item.name) {
...
const getEmptyAltCount = (richtext, field) => {
  if (richtext) {
    const occurences = richtext.split('\n').filter((x) => x.includes('![](')).length;
...
const increaseCounter = (base, field) => {
  ...
  if (richtext) {
    ...
    const wordsNotCleaned = html.replace(/<\/?[^>]+(>|$)/g, '').replace('\n', ' ').split(' ');
    const words = wordsNotCleaned.filter((x) => x !== '' && x !== '\n');
...
const recursiveSearch = (obj, searchKey, blackList, results = [], imageNames = []) => {
  ...
  Object.keys(obj).forEach((key) => {
    ...
    if (key === searchKey && typeof value !== 'object') {
      altTexts.push(value);
      names.push(obj['name']);
    } else if (typeof value === 'object' && !blackList.includes(key) && !_.isNull(value)) {
...
const getAltCheck = (contentType, modifiedData) => {
  ...
  const { altTexts, names } = recursiveSearch(modifiedData, 'alternativeText', relations);
  const altCount = altTexts.length;
  const intersection = altTexts.filter((x) => names.includes(x)).length - altCount;

...
describe('mktlng | middlewares | extendCTBInitialDataMiddleware', () => {
  describe('the action type matches "ContentTypeBuilder/FormModal/SET_DATA_TO_EDIT"', () => {
    ...
    it('forwards the action when the action.data.pluginOptions.mktlng.localized path exists', () => {
      const extendCTBInitialDataMiddleware = middleware();
      const next = jest.fn();
      const action = {
        type: 'ContentTypeBuilder/FormModal/SET_DATA_TO_EDIT',
        modalType: undefined,
        actionType: undefined,
        data: { pluginOptions: { mktlng: { localized: false } } },
      };
    ...
    it('modifies the data.pluginOptions in the action when it already exists', () => {
      ...
      expect(next).toBeCalledWith({
        ...
        actionType: 'edit',
        data: {
          pluginOptions: { mktlng: { localized: false }, somePluginThings: true },
        },
      });
    });
  });

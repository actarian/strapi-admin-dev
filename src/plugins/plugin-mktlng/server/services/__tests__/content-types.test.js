...
describe('content-types service', () => {
  describe('isLocalizedContentType', () => {
    test('Checks for the i18N option', () => {
      expect(isLocalizedContentType({ pluginOptions: { mktlng: { localized: false } } })).toBe(false);
      expect(isLocalizedContentType({ pluginOptions: { mktlng: { localized: true } } })).toBe(true);
    });

    test('Defaults to false', () => {
      expect(isLocalizedContentType({})).toBe(false);
      expect(isLocalizedContentType({ pluginOptions: {} })).toBe(false);
      expect(isLocalizedContentType({ pluginOptions: { mktlng: {} } })).toBe(false);
    });
  });
    ...
    test('fill non localized attributes', () => {
      ...
      const modelDef = {
        attributes: {
          ...
          d: {},
          e: {},
          la: { pluginOptions: { mktlng: { localized: true } } },
          lb: { pluginOptions: { mktlng: { localized: true } } },
          lc: { pluginOptions: { mktlng: { localized: true } } },
          ld: { pluginOptions: { mktlng: { localized: true } } },
          le: { pluginOptions: { mktlng: { localized: true } } },
        },
      };

 ...
 */
const LanguageSelect = React.memo(({ value, onClear, onLocaleChange, error }) => {
  ...
  return (
    <Combobox aria-busy={ isLoading } error={ error }
      label={ formatMessage({ id: getTrad('settings.locales.modal.locales.label'), defaultMessage: 'Locales' }) }
      placeholder={ formatMessage({ id: 'components.placeholder.select', defaultMessage: 'Select' }) } value={ computedValue }

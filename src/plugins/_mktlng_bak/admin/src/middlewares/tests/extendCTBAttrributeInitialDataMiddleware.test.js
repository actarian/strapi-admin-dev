...
describe('mktlng | middlewares | extendCTBAttributeInitialDataMiddleware', () => {
  ...
  describe('should forward when the type is ContentTypeBuilder/FormModal/SET_ATTRIBUTE_DATA_SCHEMA', () => {
    ...
    it('should forward if the mktlng is not activated is not contentType', () => {
      const middleware = extendCTBAttributeInitialDataMiddleware();
      const action = {
        forTarget: 'contentType',
        attributeType: 'text',
      ...
      const getState = jest.fn(() => ({
        'content-type-builder_dataManagerProvider': {
          modifiedData: {
            contentType: { schema: { pluginOptions: { mktlng: { localized: false } } } },
          },
        },
      }));
  ...
  it('should add the action.pluginOptions if the type is correct and mktlng is activated', () => {
    const middleware = extendCTBAttributeInitialDataMiddleware();
    const action = {
      forTarget: 'contentType',
      attributeType: 'text',
    ...
    const getState = jest.fn(() => ({
      'content-type-builder_dataManagerProvider': {
        // mktlng is activated
        modifiedData: {
          contentType: { schema: { pluginOptions: { mktlng: { localized: true } } } },
        },
      },
    }));
    ...
    expect(next).toBeCalledWith({
      ...action,
      options: { pluginOptions: { mktlng: { localized: true } } },
    });
  });
  ...
  it('should modify the options.pluginOptions when it exists', () => {
    ...
    expect(next).toBeCalledWith({
      ...action,
      options: { pluginOptions: { pluginTest: { ok: true }, mktlng: { localized: true } } },
    });
  });
});

...
const fetchDefaultLocalesList = async toggleNotification => {
  try {
    const data = await request('/mktlng/iso-locales', { method: 'GET' });
    return data;
  } catch (error) {
...
const useDefaultLocales = () => {
  ...
  const { isLoading, data } = useQuery('default-locales', () =>
    fetchDefaultLocalesList(toggleNotification).then(data => {
      notifyStatus(
        formatMessage({ id: getTrad('settings.locales.modal.locales.loaded'), defaultMessage: 'The locales have been successfully loaded.' })
      );
      return data;
    })
  );

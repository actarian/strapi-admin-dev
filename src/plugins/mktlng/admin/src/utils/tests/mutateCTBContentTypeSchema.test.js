...
describe('mktlng | utils ', () => {
  describe('mutateCTBContentTypeSchema', () => {
    it('should forward the data the pluginOptions.mktlng.localized key does not exist in the content type', () => {
      const data = { pluginOptions: { test: true } };

      expect(mutateSchema(data)).toEqual(data);
    });

    it('should remove the pluginOptions.mktlng key from the content type schema', () => {
      const ctSchema = {
        pluginOptions: {
          pluginA: { foo: 'bar' },
          mktlng: { localized: false },
          pluginB: { foo: 'bar' },
        },
        kind: 'test',
        attributes: {
    ...
    it('should return the data if the initial schema already has mktlng enabled', () => {
      const ctSchema = {
        pluginOptions: {
          pluginA: { foo: 'bar' },
          mktlng: { localized: true },
    ...
    it('should set the pluginOptions.mktlng.localized to true an all attributes', () => {
      const nextSchema = {
        pluginOptions: { pluginA: { ok: true }, mktlng: { localized: true } },
        attributes: {
          cover: { type: 'media', pluginOptions: { pluginA: { ok: true } } },
          name: {
            type: 'text',
            pluginOptions: { pluginA: { ok: true }, mktlng: { localized: false } },
          },
          price: {
            type: 'text',
          },
  ...
  describe('mktlng addLocalisationToFields', () => {
    ...
    it('should keep the pluginOptions for each attribute and enable the mktlng.localized value', () => {
      const attributes = {
        foo: { type: 'text', pluginOptions: { pluginA: { ok: true } }, required: true },
        bar: { type: 'text', pluginOptions: { mktlng: { localized: false } } },
      };
      ...
      const expected = {
        ...
        bar: { type: 'text', pluginOptions: { mktlng: { localized: true } } },
      };

      expect(addLocalisationToFields(attributes)).toEqual(expected);
    });

    it('should enable the pluginOptions.mktlng.localized value for each attribute', () => {
      const attributes = {
  ...
  describe('disableAttributesLocalisation', () => {
    it('should remove the pluginOptions.mktlng for all attributes', () => {
      const attributes = {
        foo: {
          type: 'text',
          pluginOptions: { pluginA: { ok: true }, mktlng: { localized: true } },

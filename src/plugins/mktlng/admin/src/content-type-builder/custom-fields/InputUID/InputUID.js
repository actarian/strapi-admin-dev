...
export function InputUID({ attribute, contentTypeUID, description, disabled, error, intlLabel, labelAction, name, onChange, value, placeholder, required }) {
  ...
  generateUid.current = async (shouldSetInitialValue = false) => {
    ...
    try {
      const { data: { data } } = await axiosInstance.post(requestURL, { contentTypeUID, field: name, data: modifiedData, });
  ...
  async function checkAvailability() {
    ...
    try {
      const { data } = await axiosInstance.post(requestURL, { contentTypeUID, field: name, value: value ? value.trim() : '' });

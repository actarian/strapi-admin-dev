...
import { LabelAction } from './edit-view/LabelAction/LabelAction';

function getPathToContentType(pathArray) {
  return ['contentType', ...pathArray];
}

export function enhanceRelationLayout(layout, locale) {
  return layout.map(current => {
...
export function enhanceEditLayout(layout) {
...
export function enhanceComponentsLayout(components, locale) {
...
export function enhanceComponentLayoutForRelations(layout, locale) {
  return layout.map(row => {
    const enhancedRow = row.reduce((p, field) => {

...
const editLocale = async (id, payload, toggleNotification) => {
  try {
    const data = await request(`/mktlng/locales/${id}`, { method: 'PUT', body: payload });
    toggleNotification({ type: 'success', message: { id: getTrad('settings.locales.modal.edit.success') } });
    return data;
  } catch {

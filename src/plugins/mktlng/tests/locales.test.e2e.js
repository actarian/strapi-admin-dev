...
describe('CRUD locales', () => {
  ...
  beforeAll(async () => {
    ...
    localeService = strapi.plugin('mktlng').service('locales');
  });
  ...
  describe('Update', () => {
    test('Can update the name of a locale', async () => {
      ...
      let res = await rq({
        url: `/mktlng/locales/${data.locales[1].id}`,
        method: 'PUT',
        body: localeUpdate,
      });
    ...
    test('Cannot update the code of a locale (without name)', async () => {
      ...
      let res = await rq({
        url: `/mktlng/locales/${data.locales[0].id}`,
        method: 'PUT',
        body: localeUpdate,
      });
  ...
  describe('Delete', () => {
    ...
    test('Delete a locale and entities in this locale', async () => {
      const { body: frenchProduct } = await rq({
        ...
        method: 'POST',
        qs: { plugins: { mktlng: { locale: 'fr-FR' } } },
      ...
      await rq({
        url: '/content-manager/collection-types/api::product.product',
        method: 'POST',
        qs: { plugins: { mktlng: { locale: 'en', relatedEntityId: frenchProduct.id } } },
        body: { name: 'product name' },
      });
    ...
    test('Cannot delete not found locale', async () => {
      let res = await rq({
        url: `/mktlng/locales/${data.deletedLocales[0].id}`,
        method: 'DELETE',
      });
